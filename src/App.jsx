...
function NicknameGate({ onLogin }) {
  ...
  const handleVideoTimeUpdate = (e) => {
    ...
    const fadeTime = 0.5 // 淡入淡出持续时间（秒）
    let opacity = 0.8
    if (cur < fadeTime) {
      opacity = (cur / fadeTime) * 0.8
    } else if (cur > dur - fadeTime) {
      opacity = ((dur - cur) / fadeTime) * 0.8
    }
  ...
  return (
    <div style={{
      ...
      {/* 视频背景：利用 JS 精确同步每一帧的透明度，确保循环点完美淡出 */}
      <video
      ...
      {/* 模糊遮罩 */}
      <div style={{ position: 'absolute', inset: 0, backgroundColor: 'rgba(8, 6, 13, 0.3)', backdropFilter: 'blur(0px)', zIndex: 0 }} />
